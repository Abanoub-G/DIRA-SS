...
DATASET_DIR              = '../datasets/CIFAR10/'#'../datasets/CIFAR10/'#'../../NetZIP/datasets/TinyImageNet/' #'../datasets/CIFAR100/' # '../../NetZIP/datasets/ImageNet/imagenet-object-localization-challenge/ILSVRC/Data/CLS-LOC' 
...
MAX_SAMPLES_NUMBER = 102#5#102#6#102#120 #220
N_T_Initial        = 1#1#5#1
N_T_STEP = 2#100#5#2#5#2 #16

Number_of_runs = 1#200#1#30#100
...
def main():
	...
	fix_seeds_flag = False #True
	experiment_number = 50
	fix_batch_noramlisation = True
	Swap_layers_starting_from = 3#5 # 2 : From layers 2  |  3 : From layers 3  |  4 : From layers 4  |  5 : For output layers only
	...
	model_multi_before_fine_tuning = copy.deepcopy(model_multi)
	model_multi_before_fine_tuning.use_classification_head()
	...
	for run_number in range(0,Number_of_runs):
		print("=========================")
		print("Repeat number = ",run_number, " / ", Number_of_runs)
		print("=========================")
		...
		for noise_type in NOISE_TYPES_ARRAY:
			...
			print("(with finetuning) Cls Accuray on " + noise_type +" dataset = ",initial_A_T)
			# # Testing herrrrrrrrrrrrrrrreee
			# _, initial_A_T    = top1Accuracy(model=model_multi_before_fine_tuning, test_loader=testloader_c_clas, device=device, criterion=None)
			# print("(without finetuning)Cls Accuray on " + noise_type +" dataset = ",initial_A_T)
		...
		results_log.write_file("exp"+str(experiment_number)+"_"+str(run_number)+".txt")
