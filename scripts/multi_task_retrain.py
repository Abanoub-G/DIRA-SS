...
from utils.dataloaders import pytorch_dataloader, pytorch_dataloader_with_rotation, cifar_c_dataloader, imagenet_c_dataloader, samples_dataloader_iterative, augmented_samples_dataloader_iterative, augmented_samples_dataloader_iterative_imagenet
...
def main():
	...
	trainloader, testloader = pytorch_dataloader_with_rotation(dataset_name=DATASET_NAME, dataset_dir=DATASET_DIR, images_size=32, batch_size=64)
	print("Progress: Dataset Loaded.")

	# accuracies = []

	# Evaluate model
	_, eval_accuracy, eval_rot_accuracy = top1Accuracy_rotation(model=model, test_loader=testloader, device=device, criterion=None)
	print("Model Accuray on original dataset = ",eval_accuracy)
	print("Model Rotation Prediciton Accuray on original dataset = ",eval_rot_accuracy)
